<template>
  <div>
    ...
    <p>
      基本用法。可以使用
      <code>v-model</code> 双向绑定数据。 单选时,
      <code>value</code>
      只接受字符串和数字类型，多选时，只接受数组类型，组件会自动根据
      <code>ivue-option</code>的 <code>value</code>来返回选中的数据。
      在展开选择器后，可以使用键盘的 <code>up</code>和
      <code>down</code>快速上下选择，按下 <code>Enter</code>选择，按下
      <code>Esc</code>收起选择器。
    </p>
    ...
    <p>
      通过给
      <code>ivue-select</code>设置属性 <code>disabled</code>禁用整个选择器
    </p>
    <p>
      通过给
      <code>ivue-option</code>设置属性 <code>disabled</code>可以禁用当前项
    </p>
    ...
    <p>
      对于选项显示内容的逻辑：优先显示
      <code>slot</code> 内容，如果没有定义 <code>slot</code>，则显示
      <code>label</code>的值
    </p>
    ...
    <p>
      设置
      <code>prefix</code> 属性或 <code>slot</code> 可以在
    ...
    <p>
      ...
      属性可以开启搜索模式。单选和多选都支持搜索模式。 多选搜索时， 可以使用键盘
      <code>Delete</code>
      快捷删除最后一个已选项
    </p>
    ...
    <p>
      远程搜索需同时设置
      <code>filterable</code>、 <code>search-method</code>、
    ...
    <p>
      ...
      <code>@on-set-default-options（仅用于 multiple 模式）</code
      >，可以为远程搜索设置默认的
      <code>label</code>
    </p>
...
const propsData = ref([
  ...
  },
  {
    ...
    illustrate: '过滤输入预设值，注意需开启<code>filterable</code>',
    type: 'String',
    value: '-',
    default: '-',
  },
  ...
  {
    name: 'default-label',
    illustrate: '远程搜索时，显示默认 <code>label</code>，详见示例',
  ...
  },
  {
    ...
    illustrate: '隐藏 <code>tag</code> 时显示的内容，参数是剩余项数量',
    type: 'Function',
    value: '-',
    default: '-',
  },
  {
    name: 'allow-create',
    illustrate: '是否允许用户创建新条目，需开启 <code>filterable</code>',
    type: 'Boolean',
    value: '-',
    default: 'false',
  },
  ...
  {
    name: 'capture',
    illustrate: '是否开启 <code>capture</code> 模式，也可通过全局配置',
    type: 'Boolean',
    value: '-',
    default: 'true',
  },
