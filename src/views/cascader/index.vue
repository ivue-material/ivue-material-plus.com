<template>
    <div>
        ...
        <p>
            ...
            <code>data</code>，每项数据至少包含
            <code>value</code>、
            <code>label</code>两项，子集为
            <code>children</code>，以此类推
        </p>
        <p>
            <code>value</code> 为当前选择的数据，如： ['1', '2'] ，按照级联顺序依次排序，使用
            <code>v-model</code> 进行双向绑定
        </p>
        ...
        <p>
            其中第一个参数
            <code>labels</code> 是当前选择的
            <code>label</code>的集合，第二个参数
            <code>selectedData</code> 是当前选择的数据集合，可以利用它们组合出你想要显示的内容
        </p>
        ...
        <p>
            <code>load-data</code> 的第二个参数为回调，如果执行，则会自动展开当前项的子列表
        </p>
...
const propsData = ref([
    ...
    },
    {
        ...
        illustrate:
            '当此项为 <code>true</code> 时，点选每级菜单选项值都会发生变化',
        type: 'Boolean',
        default: 'false',
    },
    {
