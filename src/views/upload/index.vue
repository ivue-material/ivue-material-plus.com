<template>
  <div class="upload-wrapper">
    <h1>Upload 上传</h1>
    <p>
      用于将本地的图片或文件上传至服务器，并在上传过程中展示预览图和上传进度。目前
      Uploader 组件不包含将文件上传至服务器的接口逻辑，该步骤需要自行实现
    </p>
    <h2>基本用法</h2>
    <p>
      最基本用法，点击上传，一次选择一个文件,文件上传完毕后会触发
      <code>after-read</code> 回调函数，获取到对应的 <code>file</code> 对象
    </p>
    ...
    <p>
      ...
      <code>uploading</code> 表示上传中， <code>failed</code> 表示上传失败，
    ...
    <p>
      ...
      <code>file.type</code> 区分文件类型，返回 <code>true</code> 表示超出限制，
    ...
    <p>
      ...
      <code>false</code> 表示校验失败。支持返回 <code>Promise</code> 对
    ...
    <p>
      在
      <code>v-model</code> 数组中设置单个预览图片属性， 支持
      <code>imageFit</code>, <code>deletable</code>, <code>previewSize</code>,
    ...
    <p>
      设置属性
      <code>type</code> 为 <code>drag</code>,可以拖拽上传,通过设置
    ...
    <ivue-card class="card"
      >注意：accept、capture 和 multiple 为浏览器 input
      标签的原生属性，移动端各种机型对这些属性的支持程度有所差异，因此在不同机型和
      WebView 下可能出现一些兼容性问题。</ivue-card
    >
    ...
    <h4>
      <p>
        <code>before-read</code>、 <code>after-read</code>、
    ...
    <h4>
      <p>
        通过
        <code>ref</code> 可以获取到 <code>Uploader</code> 实例并调用实例方法
      </p>
    </h4>
    ...
    <ivue-card title="Uploader 在部分安卓机型上无法上传图片？">
      ...
      <ul class="ul">
        ...
        <li class="li">
          ...
          <code>webp</code> 或 <code>heic</code> 格式
        </li>
    ...
    <ivue-card title="Uploader 在部分安卓机型上无法上传图片？">
      <p>
        ...
        <a
          ...
          >compressorjs</a
        >
        或其他开源库进行处理。 compressorjs
        是一个开源的图片处理库，提供了图片压缩、图片旋转等能力。
      </p>
      <h4>示例</h4>
    ...
    <ivue-card>
      <p>
        目前 Chrome、Safari 等浏览器不支持展示 HEIC/HEIF
        格式的图片，因此上传后无法在 Uploader 组件中进行预览
      </p>
      <p>
        [HEIF] 格式的兼容性请参考
...
const propsData = ref([
  ...
  },
  {
    ...
    type: 'String',
    value:
      '<code>contain</code> | <code>cover</code> | <code>fill</code> | <code>none</code> | <code>scale-down</code>',
    default: 'cover',
  },
  {
