<template>
    <div>
        ...
        <p>
            对于自定义指令
            <code>v-loading</code>，只需要绑定
            <code>boolean</code> 值即可
        </p>
        <p>
            通过添加
            <code>body</code> 修饰符，可以使遮罩插入至
            <code>Dom</code> 中的
            <code>body</code> 上
        </p>
        ...
        <p>
            ...
            <code>ivue-loading-text</code> 属性，其值会被渲染为加载文案，并显示在加载图标的下方
        </p>
        <p>
        ...
        <p>
            当使用指令方式时，全屏遮罩需要添加
            <code>fullscreen</code>修饰符（遮罩会插入至
            <code>body</code> 上）
        </p>
        <p>
            此时若需要锁定屏幕的滚动，可以使用
        ...
        <p>
            其中
            <code>options</code>参数为
            <code>Loading</code> 的配置项，具体见下表
        </p>
        <p>
        ...
        <ivue-table :data="optionsData" border>
            ...
            <ivue-table-column prop="default" label="默认值" width="200"></ivue-table-column>
        </ivue-table>
        <h2>Directives</h2>
