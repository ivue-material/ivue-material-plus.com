<template>
  <div>
    ...
    <p>
      ...
      将默认监听
      <code>slot</code>
      插槽的变化设置是否可以自动滚动， 而且每次数据变化将重新开始滚动
    </p>
    ...
    <p>
      ...
      <code>arrow</code>
      属性显示切换箭头显示时机，
      <code>hover(鼠标悬停)</code> 、 <code>always(一直显示)</code> 、
      <code>outside(外部显示)</code>， 设置 <code>left-arrow</code> 或
...
const propsData = ref([
  ...
  },
  {
    ...
    type: 'String',
    value:
      '<code>hover</code> | <code>always</code> | <code>never</code> | <code>outside</code>',
    default: 'always',
  },
  {
    name: 'data-list',
