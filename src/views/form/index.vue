<template>
    <div class="form">
        ...
        <p>
            ...
            属性可以改变表单域标签的位置，可选值为
            <code>top、left、right</code>，
            <code>top</code> 会置于表单域顶部
        </p>
        ...
        <ivue-table :data="formMethods" border>
            <ivue-table-column prop="name" label="名称" width="130"></ivue-table-column>
        ...
        <ivue-table :data="formItemSlots" border>
            <ivue-table-column prop="name" label="名称" width="130"></ivue-table-column>
            <ivue-table-column prop="illustrate" label="说明"></ivue-table-column>
            <ivue-table-column prop="value" label="插槽作用域" width="100"></ivue-table-column>
        </ivue-table>
    </div>
</template>
...
const formMethods = ref([
    {
        ...
        value: '(callback: (isValid: <code>是否通过验证</code>, invalidFields: <code>无效字段</code>)) => Promise',
    },
    {
        name: 'validateField',
        illustrate: '验证具体的某个字段',
        value: '(props: <code>需要验证的字段</code>, (callback: (isValid: <code>是否通过验证</code>, invalidFields: <code>无效字段</code>) => Promise',
    },
    {
        name: 'resetFields',
...
const formEvents = ref([
    {
        ...
        value: '(prop: <code>验证的prop字段</code>, isValid: <code>是否通过验证</code>, message: <code>返回的验证提示信息</code>) => void',
    },
]);
...
const formItemProps = ref([
    ...
    },
    {
        name: 'label-width',
        illustrate:
            '标签宽度，例如 <code>50px</code>, 可以使用 <code>auto</code>',
        type: 'String | Number',
        default: '-',
    },
    {
