<template>
  <div>
    ...
    <p>
      使用
      <code>ivue-alignment-[horizontal]-[vertical]</code> 类，例如
      <code>ivue-alignment-top-center</code
      >。对齐适用于父元素，并将影响所有子项的位置.
    </p>
    <doc-markdown :code="code.alignment">
      <template #demo>
        <alignment></alignment>
      </template>
    </doc-markdown>
    <h2>尺寸</h2>
    <p>
      布局系统根据项目数量计算子项大小。但是如果你想为每个项目设置尺寸。只需在项目中设置一个
      <code>ivue-size-[amount]</code> ，例如 <code>ivue-size-30</code> 。总是以
      <code>％</code>表示的大小可以是 <code>5</code>的倍数，也可以接受值
      <code>33</code>和 <code>66</code>。
    </p>
    ...
    <h2>响应</h2>
    <p>
      使用流体布局，你可以根据断点名称更改它们，加入类
      <code>ivue-[breakpoint]-size-[amount]</code>
      布局引擎将发挥神奇作用。尝试调整浏览器的大小：
    </p>
    <doc-markdown :code="code.response">
      <template #demo>
        <response></response>
      </template>
    </doc-markdown>
    <h2>隐藏元素</h2>
    <p>
      在较小的屏幕上隐藏元素的需求是相当普遍的。你可以使用这些
      <code>ivue-[breakpoint]-hide</code> 类来做到这一点：
    </p>
    <doc-markdown :code="code.hide">
      <template #demo>
        <hide></hide>
      </template>
    </doc-markdown>
...
import Size from '@/components/layout/size.vue';
import Response from '@/components/layout/response.vue';
import Hide from '@/components/layout/hide.vue';
