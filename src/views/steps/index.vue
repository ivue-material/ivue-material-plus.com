<template>
  <div>
    <h1>Steps 步骤条</h1>
    <p>组件用于显示步骤进度，引导用户按流程完成任务</p>
    <h2>基本用法</h2>
    <p>
      基本用法，组件会根据
      <code>currentStep</code>自动判断各步骤状态
    </p>
    ...
    <p>
      通过设置
      <code>Step</code> 的 <code>icon</code> 属性可以自定义图标
    </p>
    <doc-markdown :code="code.default">
      <template #demo>
        <icon></icon>
    ...
    <doc-markdown :code="code.nextSwitch">
      <template #demo>
    ...
    <p>
      设置
      <code>Steps</code> 的属性 <code>status</code> 为
      <code>error</code> 指定当前步骤状态
    </p>
    ...
    <p>
      设置属性
      <code>direction</code> 为 <code>vertical</code> 在垂直方向展示
    </p>
    ...
    <p>
      自定义内容对其方式，
      <code>right</code>文字向右, <code>bottom-center</code>底部居中,
      <code>bottom</code>底部
    </p>
    <doc-markdown :code="code.textDirection">
      <template #demo>
...
import Direction from '@/components/steps/direction.vue';
...
const propsStepsData = ref([
  ...
  },
  {
    ...
    type: 'String',
    value:
      '<code>wait</code> | <code>process</code> | <code>finish</code> | <code>error</code>',
    default: 'process',
  },
  {
    name: 'space',
  ...
  },
  {
    ...
    type: 'String',
    value:
      '<code>right</code> | <code>bottom-center</code> | <code>bottom</code>',
    default: '-',
  },
]);
